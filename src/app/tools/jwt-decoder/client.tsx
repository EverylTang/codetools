...
export default function JwtDecoderClient() {
  ...
  const result = useMemo(() => {
    ...
    if (parts.length !== 3) {
      return { parts, parsed: null, error: "Invalid JWT: expected 3 parts (header.payload.signature)" };
    }
    ...
    try {
      ...
      return { parts, parsed: null, error: "Failed to decode JWT. Check that the token is valid." };
    }
  }, [token]);
  ...
  return (
    <div className="flex flex-col flex-1 gap-4">
      ...
      <div className="flex flex-col">
        <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">JWT Token</label>
        <textarea
          value={token}
      ...
      {result.parts.length === 3 && (
        <div className="flex gap-2">
          <div className="flex-1 p-2 rounded bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
            <div className="text-[10px] font-semibold text-blue-600 dark:text-blue-400 mb-1">HEADER</div>
          ...
          <div className="flex-1 p-2 rounded bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800">
            <div className="text-[10px] font-semibold text-purple-600 dark:text-purple-400 mb-1">PAYLOAD</div>
          ...
          <div className="flex-1 p-2 rounded bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
            <div className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 mb-1">SIGNATURE</div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex-1 flex flex-col">
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Header</label>
              <button
          ...
          <div className="flex-1 flex flex-col">
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Payload</label>
              <button
      ...
      {result.parsed && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            { label: "Algorithm", value: result.parsed.header?.alg || "N/A" },
            { label: "Type", value: result.parsed.header?.typ || "JWT" },
            { label: "Issued At", value: result.parsed.payload?.iat ? new Date(result.parsed.payload.iat * 1000).toISOString() : "N/A" },
            { label: "Expires", value: result.parsed.payload?.exp ? new Date(result.parsed.payload.exp * 1000).toISOString() : "N/A" },
            { label: "Subject", value: result.parsed.payload?.sub || "N/A" },
